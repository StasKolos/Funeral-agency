...
const FeedbackModal = ({ isOpen, handleClose }: FeedbackModalProps) => {
    ...
    const isFieldFilled = (fieldName: keyof FeedbackFormData) =>
        formValues[fieldName] && formValues[fieldName].trim().length > 0;
    ...
    const onSubmit = async (data: FeedbackFormData) => {
        try {
            ...
            await axios.post('https://api.emailjs.com/api/v1.0/email/send', requestData, {
                headers: {
            ...
            toast.error(
                'Произошла непредвиденная ошибка. Обратитесь к нам по указанному номеру телефона!',
            );
        } finally {
            setIsLoading(false);
            handleClose();
        ...
        <>
            ...
            >
                ...
                >
                    ...
                    >
                        <label htmlFor={'question'}>Какой вопрос вас интересует ?</label>
                        <textarea
                            id={'question'}
                            rows={5}
                            {...register('question')}
